...
import {upper, currency} from 'utils/masking';
...
export default class FormLaptop extends Component {
  ...
  componentWillMount = () => {
    const { selectedItem, validateSelectedItem, initialize } = this.props;

    if (validateSelectedItem(selectedItem)) {
      initialize({ merk: selectedItem.merk });
      setTimeout(() => {
        initialize({ merk: selectedItem.merk, type: selectedItem.item });
      }, 200);
    }
  }
  ...
  handleChangeMerk = (value) => {
    ...
    setTimeout(() => {
      if (value) {
        ...
        dispatch(change('formLaptop', 'type', null));
      }
    }, 100);
  };
    ...
    return (
      <div style={style.wrap}>
        <form onSubmit={handleSubmit}>
          ...
          <Field name="os" component={SelectField} floatingLabelText="Sistem Operasi" maxHeight={200} fullWidth>
            {this.renderSelectField(formData.os)}
          </Field>
          <Field name="capacity" component={TextField} floatingLabelText="Kapasitas Harddisk" normalize={upper} ref="capacity" withRef fullWidth />
