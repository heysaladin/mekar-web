...
import * as dataActions from 'redux/modules/public/articles';
  ...
  getLayout = () => {
    ...
    if (data) {
      ...
      data.map(
        (dataMap) => {
          if (
          this.filterDot(dataMap.articleId) === '.' ||
          this.filterDot(dataMap.title) === '.' ||
          this.filterDot(dataMap.category) === '.'
          ) {
            layoutSelected = true;
          } else {
